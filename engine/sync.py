...
No provider credentials or raw errors belong in these durable ledgers.
"""
import hashlib
import json
import os
...
def dispatch(state, actor, mode, inputs, *, max_cost="0.10", sender=None):
    ...
    with transaction.atomic():
        ...
        spent = sum((r.cost_usd if r.cost_usd is not None else r.max_cost_usd) for r in
                    ScrapeRequest.objects.filter(state__company_id=state.company_id, created_at__date=timezone.localdate()))
        ...
        request = ScrapeRequest.objects.create(state=state, key=key, actor=actor, mode=mode, inputs=inputs, max_cost_usd=cap)
    ...
    try:
        remote = sender() if sender else apify.api("POST", f"/acts/{actor.replace('/', '~')}/runs",
            json=inputs, params={"timeout":300, "maxTotalChargeUsd":float(cap)})["data"]
        ...
        request.status = "failed" if isinstance(exc, apify.ApifyError) and not exc.uncertain else "unknown"
        request.result = {"error":"Providerstarten misslyckades eller är obekräftad. Kontrollera Apify före nytt försök.",
        ...
        raise apify.ApifyError(request.result["error"], uncertain=request.status == "unknown") from exc
...
def analysis(company, key, model, work, *, scrape_request_id=None):
    ...
    with transaction.atomic():
        ...
        if memo:
            ...
            if memo.status == "rejected" and (memo.last_attempt_at or memo.created_at) <= timezone.now()-timedelta(hours=23):
                ...
                raise ValueError("Analysen har redan startats men kunde inte bekräftas. Kontrollera analysloggen före nytt betalt försök.")
        limit = min(50, max(0, int(os.environ.get("INTELLIGENCE_DAILY_ANALYSES", "12"))))
        from django.db.models import Q
        today = AnalysisMemo.objects.filter(company=company).filter(Q(last_attempt_at__date=timezone.localdate()) | Q(last_attempt_at=None, created_at__date=timezone.localdate()))
    ...
    try:
        ...
        AnalysisMemo.objects.filter(pk=memo.pk).update(status="rejected" if isinstance(status, int) and 400 <= status < 500 else "unknown")
        raise
    AnalysisMemo.objects.filter(pk=memo.pk).update(status="completed", result=result)
    return result
