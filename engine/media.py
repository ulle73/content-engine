...
def store_asset(company, data, *, job=None, index=0, alt_text=""):
...
def create_job(run, *, token, kind, brief, count=2, shape="portrait", source=None):
    check_storage()
    ...
    with transaction.atomic():
        ...
        return MediaGeneration.objects.create(id=token, run=locked, kind=kind, provider="openai" if kind == "image" else "higgsfield",
                                               brief=brief, prompt=generation_prompt(run, brief, kind), parameters=params, source_asset=source)
...
def remove_asset(asset):
    with transaction.atomic():
        ...
        if locked.used_at or locked.content_runs.exists() or locked.variations.filter(status__in=ACTIVE).exists():
