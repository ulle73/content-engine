...
COUNTRIES = {"SE":"Sverige", "US":"USA", "GB":"Storbritannien", "NO":"Norge", "DK":"Danmark", "FI":"Finland",
             "DE":"Tyskland", "FR":"Frankrike", "CA":"Kanada", "AU":"Australien", "NL":"Nederländerna"}
...
def start(account):
    ...
    if not account.page_id or not account.page_id.isdigit():
        raise apify.ApifyError("Ange annonsörens Facebook-sid-id eller en Ads Library-länk med view_all_page_id. En vanlig sidlänk kan ge fel annonsörer.")
    state = account_state(account)
    existing = state.requests.filter(status__in=OPEN).first()
    if existing:
        return existing
    if state.coverage == "limited":
        raise apify.ApifyError("Resultatgränsen nåddes. Sparade annonser finns kvar; kontrollera täckningen innan mer hämtas.")
    now = timezone.now()
    ...
    inputs = {"searchQueries":[f"https://www.facebook.com/ads/library/?view_all_page_id={account.page_id}&country={account.country}&active_status=all&ad_type=all"], "maxResults":limit, "activeStatus":"ALL",
              "countries":[account.country], "publisherPlatforms":["FACEBOOK", "INSTAGRAM"],
              "adDeliveryDateMin":since.isoformat(), "adDeliveryDateMax":until.isoformat()}
...
def normalize(row, account):
    ...
    creative = {"text":text("adText"), "headline":text("headline"), "description":text("description"),
                "cta_caption":text("ctaCaption"), "cta":text("ctaType"),
                "format":text("mediaType") or "unknown", "landing_page":safe_url(row.get("landingPageUrl")),
                "platforms":sorted(str(p) for p in (row.get("publisherPlatforms") or []) if isinstance(p, str)),
                "variant_count":row.get("creativeVariants") if isinstance(row.get("creativeVariants"), int) and not isinstance(row.get("creativeVariants"), bool) else None,
                "image_url":safe_url(row.get("imageUrl")), "video_url":safe_url(row.get("videoUrl"))}
    ...
    return {"external_id":external_id, "creative":creative, "creative_hash":fingerprint(creative),
...
def collect(request, account, *, reprocess=False):
    ...
    owners = {str(row.get("advertiserPageId")) for row in rows if isinstance(row, dict) and row.get("advertiserPageId")}
    for row in rows:
        try:
    ...
    with transaction.atomic():
        ...
        for item in {i["external_id"]:i for i in valid}.values():
            ...
            ad.save()
            AdObservation.objects.get_or_create(ad=ad, request=locked, defaults={"observed_at":observed,
                "data":{**item, "start_date":str(item["start_date"]) if item["start_date"] else None,
                         "end_date":str(item["end_date"]) if item["end_date"] else None}})
        status = "succeeded" if complete else ("partial" if valid else "failed")
        finish(locked, status=status, cost=remote.get("usageTotalUsd"), observed_at=observed,
            result={"new":created, "changed":changed, "saved":len(valid), "skipped":skipped, "excluded_other_advertisers":excluded, "capped":capped,
                    "coverage":"bounded_delivery_window", "performance_available":False})
        ...
        state.coverage = "identity_required" if not account.page_id else (("refresh_limited" if locked.mode == "refresh" else "limited") if capped else ("bounded_delivery_window" if complete else "partial"))
        if complete:
...
Föreslå en originell vinkel för vårt företag med endast våra verifierade fakta. Kopiera inte konkurrentens copy eller claims.""",
                input=json.dumps({"creative":ad.creative, "company_profile":company.profile,
                                  "current":company.current, "voice":company.voice}, ensure_ascii=False),
                text_format=AdClassification, max_output_tokens=1800)
...
def signals(company, selected=None):
    ads = CompetitorAd.objects.filter(account__company=company, account__active=True).select_related("account").order_by("-last_seen_at", "-pk")
    if selected:
    ...
    for ad in ads[:100]:
        if not ad.classification or ad.classification_hash != classification_hash(ad, company):
            continue
        # Editorial relevance only. No synthetic performance metrics or lifecycle success bonuses.
        result.append({"id":f"ad:{ad.pk}", "account":ad.account.name,
            "url":f"https://www.facebook.com/ads/library/?id={ad.external_id}", "format":ad.creative.get("format", "unknown"),
            "classification":ad.classification, "score":40, "ranker_version":"paid-editorial-v1",
            "as_of":ad.last_seen_at.isoformat(), "creative_hash":ad.creative_hash,
            "classification_hash":ad.classification_hash, "performance_available":False, "confidence":0,
