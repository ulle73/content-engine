...
export function createGitHubRefClient(token: string, request: RequestFn = fetch): GitHubRefClient {
  ...
  return {
    ...
    async setRef(repo, branch, sha) {
      await call(`${repoUrl(repo)}/git/refs/heads/${encodeURIComponent(branch)}`, { method: "PATCH", body: JSON.stringify({ sha, force: false }) });
    },
    async restoreRef(repo, branch, sha) {
      await call(`${repoUrl(repo)}/git/refs/heads/${encodeURIComponent(branch)}`, { method: "PATCH", body: JSON.stringify({ sha, force: true }) });
    },
